#!/usr/bin/env python3
"""
Script para gerar mapeamento completo da API com todas as aulas
"""
...
def generate_web_fundamentals_mapping() -> List[Dict]:
    """Gera mapeamento completo para Web Fundamentals"""
    mappings = []
...
def update_api_route():
    """Atualiza o arquivo da API com o novo mapeamento"""
    mappings = generate_web_fundamentals_mapping()
    
    # Gerar código TypeScript
    ts_code = f"""import {{ NextResponse }} from 'next/server';
...
"""
    ...
    for mapping in mappings:
        ts_code += f'    {{ globalLessonId: {mapping["globalLessonId"]}, moduleId: {mapping["moduleId"]}, lessonTitle: "{mapping["lessonTitle"]}", fileName: "{mapping["fileName"]}" }},\n'
...
"""
    ...
    # Salvar arquivo
    with open('frontend/app/api/lessons/[courseSlug]/[moduleId]/[lessonId]/route.ts', 'w', encoding='utf-8') as f:
        f.write(ts_code)
    
    print(f"✅ API atualizada com {len(mappings)} aulas do Web Fundamentals")
