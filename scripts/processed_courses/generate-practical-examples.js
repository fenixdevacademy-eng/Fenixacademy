...
plt.show()`
                ...
                'aula-02': {
                    ...
                    practicalExample: `# Exemplo prático: Análise de Dados de Funcionários
import pandas as pd
import numpy as np

# Criar dados de funcionários
funcionarios = {
    'nome': ['Ana', 'Bruno', 'Carlos', 'Diana', 'Eduardo'],
    'idade': [25, 30, 35, 28, 32],
    'salario': [5000, 6000, 7000, 5500, 6500],
    'departamento': ['TI', 'RH', 'TI', 'Vendas', 'TI'],
    'experiencia': [2, 5, 8, 3, 6]
}
...
analise_dept = df.groupby('departamento').agg({
...
funcionarios_acima_media = df[df['salario'] > salario_medio]

print(f"\\nFuncionários com salário acima da média (R$ {salario_medio:.2f}):")
print(funcionarios_acima_media[['nome', 'salario', 'departamento']])

# Correlação entre idade e salário
correlacao = df['idade'].corr(df['salario'])
print(f"\\nCorrelação entre idade e salário: {correlacao:.3f}")`
                }
            }
        }
    },
    'nodejs-apis': {
        ...
        ide: 'VS Code',
        extensions: ['Node.js', 'Express', 'MongoDB'],
        examples: {
            'modulo-01': {
                'aula-01': {
                    title: 'Introdução ao Node.js',
                    practicalExample: `// Exemplo prático: Servidor HTTP Simples
...
server.listen(PORT, () => {
    console.log(\`Servidor rodando na porta \${PORT}\`);
                ...
                'aula-02': {
                    ...
                    practicalExample: `// Exemplo prático: API REST com Express
...
let produtos = [
    ...
    { id: 2, nome: 'Mouse', preco: 50, categoria: 'Acessórios' }
];
...
function generateGenericExample(course, lessonTitle) {
    const examples = {
        'csharp': {
            basic: `// Exemplo prático: ${lessonTitle}
using System;
...
namespace ExemploPratico
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Funcionalidade implementada com sucesso!");
...
function updateLessonWithPracticalExample(coursePath, moduleNumber, lessonNumber, lessonTitle) {
    const moduleKey = `modulo-${moduleNumber.toString().padStart(2, '0')}`;
    const lessonKey = `aula-${lessonNumber.toString().padStart(2, '0')}`;
    const fileName = `${lessonKey}-${moduleKey}-${coursePath.replace('-', '-')}.md`;
...
function processAllCourses() {
    ...
    const coursesDir = 'processed_courses';
