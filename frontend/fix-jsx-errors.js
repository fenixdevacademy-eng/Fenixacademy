...
function fixJsxSyntax(content) {
    ...
    // Corrigir sintaxe JSX das props do motion.div
    ...
    // Corrigir sintaxe JSX das props do motion.div (sem espaços)
    content = content.replace(/initial=\{([^}]+)\}/g, 'initial={{$1}}');
    content = content.replace(/animate=\{([^}]+)\}/g, 'animate={{$1}}');
    content = content.replace(/transition=\{([^}]+)\}/g, 'transition={{$1}}');
...
console.log('🔧 Iniciando correção automática dos erros JSX...\n');
